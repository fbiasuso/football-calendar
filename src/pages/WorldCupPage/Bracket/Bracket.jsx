...
import { resolveBracket, isThirdPlaceSlot } from './bracketEngine.js';
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          {m.home?.logo && (
            <img src={m.home.logo} alt="" className="w-4 h-4 flex-shrink-0" onError={(e) => { e.target.style.display = 'none'; }} />
          )}
          <span className={`truncate ${m.home?.name ? 'font-medium text-gray-800' : 'text-gray-400'}`}>
            {m.home?.name || (m.isPlaceholder ? '—' : (m.roundIndex > 0 && m.away && !m.home ? 'Pendiente' : (m.isThirdPlace && !m.home ? '3° ?' : 'TBD')))}
          </span>
          {isPickedHome && (
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          {m.away?.logo && (
            <img src={m.away.logo} alt="" className="w-4 h-4 flex-shrink-0" onError={(e) => { e.target.style.display = 'none'; }} />
          )}
          <span className={`truncate ${m.away?.name ? 'font-medium text-gray-800' : 'text-gray-400'}`}>
            {m.away?.name || (m.isPlaceholder ? '—' : (m.roundIndex > 0 && m.home && !m.away ? 'Pendiente' : (m.isThirdPlace && !m.away ? '3° ?' : 'TBD')))}
          </span>
          {isPickedAway && (
            <span className="w-1.5 h-1.5 rounded-full bg-blue-500 flex-shrink-0" title="Pickeado" />
          )}
      ...
      <div className="mb-6 flex items-center gap-4 flex-wrap">
        ...
        {pickCount > 0 && (
          <span className="text-xs text-gray-400">
            {pickCount} pick{pickCount !== 1 ? 's' : ''} activo{pickCount !== 1 ? 's' : ''} — los resultados se propagan automáticamente
          </span>
        )}
      </div>
